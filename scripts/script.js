    $(document).ready(function(){
        ...
        async function loadTask(){
            try{
                ...
                tasks.filter(task=>{
                    let dueDate = new Date(task.dueDate)
                    if(curDate > dueDate && task.completed === false){  
                        task.status = 'overdue'
                        task.overDue = true 
                    }
                    return task.userId === user.id && task.status === 'pending' })
    ...
    async function displayCompleteTasks(){
        try{
                let data = await fetch('http://localhost:3000/todos') 
                let tasks = await data.json()
                tasks.filter(task=>task.userId === user.id && task.status === 'completed').forEach(task=>createTask(task))
            }
            catch(error){
                console.error("Failed to load tasks",error)
            }
    ...
    async function displayOverDueTasks(){
        try{
            ...
            tasks.filter(task=>{
                return task.userId == user.id && task.status === 'overdue'  
            }).forEach(task =>{
                createTask(task)
            })
        }
